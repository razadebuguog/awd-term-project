...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const storedDocs = typeof window !== 'undefined' ? localStorage.getItem('documents') : null;
    if (storedDocs) {
      try {
  ...
  return (
    <div
      className={`min-h-screen pt-12 pb-12 font-inter bg-[${COLOR_LIGHT_BG}] text-[${COLOR_TEXT_DARK}]`}
    >
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 p-6 rounded-2xl shadow-xl bg-white shadow-gray-300/50">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 text-sm font-bold rounded-xl bg-red-600 text-white hover:bg-red-700 transition"
          >
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-3">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-500">
              Showing {sortedDocs.length} of {documents.length} document
        ...
        <div className="space-y-4">
          {sortedDocs.map((doc) => (
            <div
              key={doc._id}
              className="flex flex-col sm:flex-row items-center p-4 rounded-xl shadow-md bg-white border border-gray-100 hover:shadow-lg transition"
            >
